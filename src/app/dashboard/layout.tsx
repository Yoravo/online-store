...
function Sidebar({
  ...
  return (
    <aside className="w-60 bg-gray-950 min-h-screen flex flex-col h-full">
...
export default function DashboardLayout({
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      {/* Desktop Sidebar */}
      <div className="hidden md:flex fixed left-0 top-0 h-full w-60 z-40">
        <Sidebar user={user} pathname={pathname} logout={logout} />
      </div>
      ...
      {mobileOpen && (
        <>
          ...
          />
          <div className="fixed left-0 top-0 h-full w-60 z-50 md:hidden flex">
            <Sidebar
              user={user}
              pathname={pathname}
              logout={logout}
      ...
      <main className="flex-1 md:ml-60 min-h-screen">
        {/* Mobile Topbar */}
        <div className="md:hidden flex items-center gap-3 px-4 py-3 bg-gray-950 border-b border-white/5 sticky top-0 z-30">
          <button
          ...
          <span className="text-white font-bold text-sm">TokoKu Seller</span>
        </div>
